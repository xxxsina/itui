...
export default {
  ...
  methods: {
    ...
    handleLink (index, column) {
      // 这里有两种方式
      // 第一 使用params传递参数，路由必须使用name引入, 获取 this.$route.params
      this.$router.push({name: 'jump/channel', params: {'goback': 'jump', 'parent_id': column.id, 'uid': column.uid, 'page': this.data.page}})
      // 第二 使用query传递参数，路由必须使用path引入, 获取 this.$route.query
      // this.$router.push({path: 'jump/channel', query: {'parent_id': column.id}})
    },
    // 修改状态
